...
void filter(Node ** ptp){
    ...
    //Tao mang luu thong tin nhan vien {full_name, freq, info}
    Count nv[MAX];
    for (int i = 0; i < count; i++)
    {   
        for (int k = 0; k < i; k++)
        {
            if (strcmp(temp1->data.first_name,nv[k].first_name) == 0 && strcmp(temp1->data.last_name,nv[k].last_name) == 0)
            {
                strcpy(nv[i].first_name,temp1->data.first_name);
                strcpy(nv[i].last_name,temp1->data.last_name);
        ...
        {   
            if (strcmp(p->data.first_name,temp1->data.first_name) == 0 && strcmp(p->data.last_name, temp1->data.last_name) == 0)
            {
                nv[i].freq ++; 
            }   
        }
    ...
    for (int i = 0; i < count; i++)
    {
        if (nv[i].freq > 1)
        {
            printf("Nhan vien co ten trung lap: %s %s, so lan lap: %d\n",nv[i].first_name, nv[i].last_name,nv[i].freq);
            a++;
            for (int k = 0; k < i; k++)
            {
                if (strcmp(nv[i].first_name,nv[k].first_name) == 0 && strcmp(nv[i].last_name,nv[k].last_name) == 0)
                {
                    goto EXIT;
                }              
            }
            printf("Danh sach nhan vien co ten '%s %s':\n",  nv[i].first_name,nv[i].last_name);
            for (int j = i; j < count; j++)
            {   
                if (strcmp(nv[j].first_name,nv[i].first_name) == 0 && strcmp(nv[j].last_name,nv[i].last_name) == 0)
                {
    ...
    if (a == 0)
    {
        printf("Khong co nhan vien nao co ten trung lap\n");
    }
